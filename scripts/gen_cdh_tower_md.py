...
tower_df = pd.read_csv('../datasets/CDH_Tower.csv',keep_default_na=False, na_values=[""])

merged_df = pd.merge(complete_df, tower_df, on="Card" or "OLD NAME")
print(merged_df)
del merged_df['OLD NAME']
del merged_df['O']
merged_df.rename(columns={'Attribute 6 Affinity': 'Affinity'}, inplace=True)

merged_df.to_csv('../datasets_output/final_cdh_tower.csv', index=False)
...
csv_file = open('../datasets_output/final_cdh_tower.csv', 'r') 
...
for i, row in enumerate(csv_reader):
    if i != 0:
        ...
        jsonfile = open('../metadatas/metadata_cdh_tower/'+ 'tower'+str(counter)+'.json', 'w')
    ...
    jsonComplete =  open('../metadatas/metadata_cdh_tower_complete'+ '.json', 'w')
